...
9. **Звёздочки / Pin to context** (`chat_minimal.html`) — пользователь может отметить любую реплику (свою, собеседника или AI-подсказку) звёздочкой, кликнув по маленькой иконке `.msg__star` сбоку от пузыря / hint-карточки. Иконка появляется при наведении на реплику, в активном состоянии становится золотой. Состояние сохраняется в `localStorage` под ключом `quistan:starred-messages`.
   - **Триггер** (`#starsTrigger`) живёт в `chat-panel__controls` рядом с микрофонами и FAB play/pause. Виден **только при наличии хотя бы одной звезды** (класс `.is-visible` навешивается JS), на бейдже — счётчик.
   - **Попап** (`#starsPopup`) открывается кликом по триггеру вверх над ним; показывает список отмеченных реплик (спикер + время + превью текста с обрезкой до 2-х строк + сохранением `<code>`-подсветки), per-item ✕ для индивидуального снятия и «Clear all» в шапке. Закрывается по клику вне / Esc.
   - **Назначение:** содержимое отмеченных реплик **всегда подмешивается в контекст LLM** (system / developer) при отправке нового сообщения — пока пользователь сам не уберёт отметку. Это способ «навсегда прибить» важные формулировки/правила к сессии без необходимости копировать их вручную каждый раз.
   - **Контракт с бэкендом** (уже закомментирован в JS): `POST /api/session/:id/stars { add: [...ids] }` / `{ remove: [...ids] }`, UI оптимистичен. Бэкенд обязан автоматически инжектить starred-реплики в контекст при каждом запросе к модели.
   - В первоначальной версии звёздочки размещались в нижней панели рядом с `Remove all stars`, но затем перенесены в чат-панель (логически принадлежат контенту чата, а не карточке аккаунта).
...
13. **Звёздочки (pin to context)** — зафиксировать как утверждённую фичу: UI в `chat_minimal.html` готов (иконка на реплике + триггер в чат-панели + попап-список), нужно явно прописать серверный контракт (эндпоинт `POST /api/session/:id/stars`) и правило автоматической инжекции pinned-реплик в контекст LLM. Это **отдельная подсистема управления контекстом**, не часть «слайсов» / «pre-flight».
...
| 02.09      | Звёздочки на репликах (`.msg__star` сбоку от пузыря) + триггер в чат-панели + попап со списком. Цель — pinned-to-context для LLM | сессия «Звёздочки на репликах чата» |
